...
def SQL_reader(w, A_min, S_min, Ix_min, design_base, unit_system):
    if design_base == "moment":
        control_value = S_min
        index_value = 3
    elif design_base == "shear":
        control_value = A_min
        index_value = 2
    else:  # deflection
        control_value = Ix_min
        index_value = 1
    ...
    for item in parameters:
        if float(item[index_value]) >= control_value:
            ...
            Sx_list.append(item[3])
            wc_list.append(item[4])
            h_list.append(item[5])
            bf_list.append(item[6])
            tw_list.append(item[7])
            tf_list.append(item[8])
